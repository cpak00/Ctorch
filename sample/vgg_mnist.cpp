...
public:
    VGG(int hidden_size = 32): Model<T>(8), pool1(2), pool2(2), pool3(2) {
        ...
        this->layer[7] = new Linear<T>(hidden_size * 4, 10);
...
int main() {
    ...
    SGD<float> optimizer(model.parameters(), model.nparameters(), 0.01, 0);
    int epoch_size = 100;
    ...
    DataLoader<float> trainloader(mnist_train, 128);
